        ...
        <div key={section.id} className="flex flex-col items-center gap-6 w-full border-b-2 border-pink-100/50 pb-10 last:border-0 last:pb-0">
          <div className="flex flex-wrap justify-center gap-4 sm:gap-8 relative">
            {section.photos.map((photo) => (
              <div
                key={photo.id}
                className={`bg-white p-2 sm:p-3 shadow-md rounded-sm w-36 h-48 sm:w-48 sm:h-56 md:w-64 md:h-72 flex flex-col items-center hover:scale-105 hover:z-10 hover:rotate-0 transition-transform ${photo.rot}`}
              >
          ...
          <p className="mt-2 px-4 text-[#FF1E56] font-caveat text-2xl sm:text-3xl md:text-4xl font-bold text-center">
            "{section.quote}"
          </p>
        </div>
      ))}
